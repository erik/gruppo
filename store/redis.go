...
func (r RedisStore) ListPostOverviews(site model.Site, prefix string, offset, limit int) ([]model.PostOverview, error) {
	k := KeyForSite(site, "slugs")

	objs, err := r.db.ZRange(k, int64(offset), int64(offset+limit)).Result()
	if err != nil {
